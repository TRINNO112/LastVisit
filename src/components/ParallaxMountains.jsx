...
// ═══════════════════════════════════════════════════════════════════════════════
// TREE COMPONENT — crisp layered pine silhouette
// ═══════════════════════════════════════════════════════════════════════════════
...
export default function ParallaxMountains() {
    ...
    const handleScroll = useCallback(() => {
        ...
        const speeds = isMobile
            ? [0.01, 0.025, 0.055, 0.10, 0.16, 0.24, 0.33, 0.44, 0.58]
            : [0.012, 0.035, 0.075, 0.14, 0.22, 0.34, 0.48, 0.65, 0.86]

        const mult = isMobile ? 0.5 : 0.82
    ...
    const hillBaseY = isMobile ? 228 : 262
    const sectionH = isMobile ? '90vh' : '135vh'
    const sectionMin = isMobile ? 460 : 660

    // ── Ground path (foreground) ─────────────────────────────────────────────
    const groundPath = isMobile
        ? 'M0,400 L0,228 Q60,210 130,224 Q210,198 295,220 Q362,200 430,218 L430,500 L0,500 Z'
    ...
    const ridgePath = isMobile
        ? 'M0,228 Q60,210 130,224 Q210,198 295,220 Q362,200 430,218'
    ...
    return (
      ...
      ══════════════════════════════════════════════════════════════════ */}
            <svg className="absolute w-0 h-0 overflow-hidden" aria-hidden="true">
                <defs>
                    {/* Moon glow removed by CSS fallback */}
                    {/* Atmospheric blur */}
                    <filter id="pm-atmos">
      ...
      ══════════════════════════════════════════════════════════════════ */}
            ...
            >
                {/* Removed ambient glow and yellow pulse rings based on feedback */}
                ...
                <svg
                    style={{
                        ...
                        animation: 'pm-moon-float 9s ease-in-out infinite',
                        display: 'block',
                        borderRadius: '50%',
                ...
                <svg className="absolute bottom-0 left-0 w-full h-full" viewBox={vb} preserveAspectRatio="xMidYMax slice" style={{ overflow: 'visible' }}>
                    <path
                        d={isMobile
                            ...
                            : 'M0,500 L0,120 Q90,60 200,110 Q288,30 408,90 Q490,20 614,82 Q696,30 816,88 Q900,22 1020,82 Q1104,35 1228,90 Q1316,50 1440,100 L1440,500 Z'
                        }
                        fill="url(#pm-m1)"
                        opacity="0.62"
                    />
                ...
                <svg className="absolute bottom-0 left-0 w-full h-full" viewBox={vb} preserveAspectRatio="xMidYMax slice" style={{ overflow: 'visible' }}>
                    <path
                        d={isMobile
                            ...
                            : 'M0,500 L0,150 Q108,80 236,140 Q338,50 458,118 Q548,42 668,108 Q752,40 872,104 Q958,42 1080,104 Q1168,52 1292,110 Q1376,75 1440,122 L1440,500 Z'
                        }
                        fill="url(#pm-m2)"
                        opacity="0.8"
                    />
      ...
      ══════════════════════════════════════════════════════════════════ */}
            ...
            >
                <svg className="absolute bottom-0 left-0 w-full h-full" viewBox={vb} preserveAspectRatio="xMidYMax slice" style={{ overflow: 'visible' }}>
                    ...
                    <path
                        d={isMobile
                            ? 'M0,500 L0,252 Q92,178 180,240 Q264,108 348,196 Q396,158 430,198 L430,500 Z'
                            : 'M0,500 L0,150 Q108,70 232,140 Q352,30 476,110 Q572,23 696,95 Q792,17 916,90 Q1014,25 1140,93 Q1240,37 1364,100 Q1412,75 1440,95 L1440,500 Z'
                        }
                        fill="url(#pm-m3)"
                    />
                    ...
                    {Array.from({ length: 12 }).map((_, i) => {
                        ...
                        return (
                            <DistantPineTree
                                key={`l7-${i}`}
                                x={x}
                                h={15} // Optimized distant tree model
                                w={5}
                                delay={i * 0.1}
                                swing={4.5 + (i % 3) * 0.5}
                                hillY={layer7Y}
      ...
      ══════════════════════════════════════════════════════════════════ */}
            ...
            >
                <svg className="absolute bottom-0 left-0 w-full h-full" viewBox={vb} preserveAspectRatio="xMidYMax slice" style={{ overflow: 'visible' }}>
                    <path
                        d={isMobile
                            ? 'M0,500 L0,262 Q72,196 152,252 Q228,158 316,226 Q380,188 430,218 L430,500 Z'
                            : 'M0,500 L0,175 Q118,95 252,170 Q374,80 500,153 Q604,83 732,153 Q832,83 964,153 Q1068,90 1200,153 Q1308,105 1440,163 L1440,500 Z'
                        }
                        fill="url(#pm-m4)"
                    />
                    ...
                    {Array.from({ length: 16 }).map((_, i) => {
                        ...
                        return (
                            <DistantPineTree
                                key={`l8-${i}`}
                                x={x}
                                h={22} // Optimized distant tree model
                                w={7}
                                delay={i * 0.15}
                                swing={3.5 + (i % 4) * 0.5}
                                hillY={layer8Y}
